<template>
  <div class="flex">
      ...
      <main class="flex justify-evenly mr-5">
        <div>
          ...
          >
            <div v-for="host in hostnames" :key="host">
              <div
                v-for="drivingData in filteredDrivingData"
                :key="drivingData"
              >
        ...
        <div>
          ...
          >
            <div v-for="host in hostnames" :key="host">
              <div
                v-for="ParkingData in filteredParkingData"
                :key="ParkingData"
              >
    ...
    <aside class="h-[500px] mt-5 w-[500px] bg-slate-200 ml-[40px]">
      <Servicelocation
        :drivingjson="propsData"
        :selected="selected"
        :selecttype="selectcardName"
      />
    </aside>
  </div>
</template>
...
const sendData = (event, data, select) => {
  console.log("click");
  propsData.value = data;
  selectcardName.value = select;
  console.log(data);
  selected.value = true;

  //   propsData.value = select;
};
