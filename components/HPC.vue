<template>
  <header class="header">
        ...
        <li
          ...
          :style="{
            backgroundColor:
              server.color !== ''
                ? server.color
                : (server.color = generateRandomColors()),
          }"
        >
    ...
    <div
      class="parent"
      :class="errorMessage === '' ? 'justify' : 'error-border'"
    >
...
const onDragDrop = (e) => {
  ...
  if (droppedElement.innerText === "") {
    if (usedServices < 10) {
      usedServices += 1;
      console.log(draggedElement.style.backgroundColor);
      let vmdata = users.value[Number(parentIndex)].VM[Number(currentIndex)];

      vmdata.server = draggedElementValue;
      vmdata.backgroundColor = draggedElement.style.backgroundColor;
      ...
      // droppedElement.classList.add("server-row");
      // console.log("error - service reached maximum limit");
      errorMessage.value = "error - service reached maximum limit";
    }
